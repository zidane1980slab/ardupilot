...
/*
  constructor
 */
AP_Baro_Revo::AP_Baro_Revo(AP_Baro &baro, AP_HAL::OwnPtr<AP_HAL::Device> dev, enum MS56XX_TYPE ms56xx_type)
...
AP_Baro_Backend *AP_Baro_Revo::probe(AP_Baro &baro,
                                       ...
                                       enum MS56XX_TYPE ms56xx_type)
{
    ...
    AP_Baro_Revo *sensor = new AP_Baro_Revo(baro, std::move(dev), ms56xx_type);
...
bool AP_Baro_Revo::_init()
{
    ...
    if (!sem || !_sem) {
         AP_HAL::panic("AP_Baro_Revo: failed to create semaphore");
    }

    if (!sem->take(HAL_SEMAPHORE_BLOCK_FOREVER)) {
        AP_HAL::panic("PANIC: AP_Baro_Revo: failed to take serial semaphore for init");
    }
    ...
    _dev->register_periodic_callback(10 * AP_USEC_PER_MSEC,
                                     FUNCTOR_BIND_MEMBER(&AP_Baro_Revo::_timer, void));
    return true;
}
...
uint16_t AP_Baro_Revo::_read_prom_word(uint8_t word)
{
    const uint8_t reg = CMD_MS56XX_PROM + (word << 1);
    uint8_t pval[2];
    if (!_dev->transfer(&reg, 1, pval, sizeof(pval))) {
        return 0;
    }
...
bool AP_Baro_Revo::_read_prom_5611(uint16_t prom[8])
{
    /*
...
bool AP_Baro_Revo::_read_prom_5637(uint16_t prom[8])
{
    /*
 ...
 * pressure.
*/
void AP_Baro_Revo::_timer(void)
{
    uint8_t next_cmd;

    if(in_progress){ // there was no IO_Completion interrupt - some went wrong
        next_cmd = _state == 0 ? ADDR_CMD_CONVERT_TEMPERATURE
    ...
    _dev->register_completion_callback(FUNCTOR_BIND_MEMBER(&AP_Baro_Revo::_ioc, void)); // IO completion interrupt
        
    in_progress=true;


    _dev->transfer(&CMD_MS56XX_READ_ADC, 1, (uint8_t *)val, sizeof(val));
    

}

void AP_Baro_Revo::_ioc(void)
{
    uint8_t next_cmd;
    uint8_t next_state;

    uint32_t adc_val = (val[0] << 16) | (val[1] << 8) | val[2];
...
//    _dev->register_completion_callback(FUNCTOR_BIND_MEMBER(&AP_Baro_Revo::_write_ioc, void)); // write will be in background                               
    _dev->register_completion_callback(NULL); // allow the bus driver to release the bus semaphore
...
exit:
    _sem->give();
    _dev->get_semaphore()->give(); // release bus semaphore

    in_progress=false;
}


void AP_Baro_Revo::_update_and_wrap_accumulator(uint32_t *accum, uint32_t val,
...
void AP_Baro_Revo::_calculate_5611()
{
    float dT;
    float TEMP;
    float OFF;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AP_Baro_Revo::_calculate_5607()
{
    float dT;
    float TEMP;
    float OFF;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AP_Baro_Revo::_calculate_5637()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AP_Baro_Revo::_calculate_5837()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
    int32_t raw_pressure = _D1;
